...
class Box extends EventEmitter {
  ...
  _readDir(base, prefix = '') {
    const results = [];
    this.context.log.debug("readDir: %s %s %s", base, results, prefix);
    return readDirWalker(this.context, base, results, this.ignore, prefix)
      .return(results)
  ...
  _checkFileStatus(path) {
    ...
    ctx.log.debug("checkFileStatus: %s", src);
...
function readDirWalker(ctx, base, results, ignore, prefix) {
  ...
  return Promise.map(readdir(base).catch(err => {
    ...
    if (stats.isDirectory()) {
      ctx.log.debug("isDirectory: %s", fullpath);
      return readDirWalker(ctx, fullpath, results, ignore, `${prefixdPath}/`);
    }
    if (!isIgnoreMatch(ctx, fullpath, ignore)) {
